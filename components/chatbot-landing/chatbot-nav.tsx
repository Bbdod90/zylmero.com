...
const LINKS = [
  ...
  { href: "#hoe-het-werkt", label: "Zo werkt het" },
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-14 max-w-[1200px] items-center justify-between gap-2 px-4 md:h-[3.75rem] md:gap-4 md:px-8">
        <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2.5 text-foreground" title="Naar homepage">
        ...
        <nav className="hidden items-center gap-0.5 xl:flex">
          {LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="rounded-lg px-2.5 py-2 text-[0.75rem] font-medium tracking-tight text-foreground transition-colors hover:bg-muted/70 lg:px-3 lg:text-[0.8125rem]"
            >
              {l.label}
            </a>
          ))}
        ...
        <div className="flex shrink-0 items-center justify-end gap-1 sm:gap-2">
          <ThemeToggle />
          <AnonymousDemoForm>
            <Button type="submit" size="sm" variant="demo" className="rounded-lg px-2.5 text-[0.7rem] sm:text-[0.8125rem]">
              Demo app
            </Button>
          </AnonymousDemoForm>
          <Button size="sm" className="rounded-lg px-3 text-[0.75rem] font-semibold sm:text-[0.8125rem]" asChild>
