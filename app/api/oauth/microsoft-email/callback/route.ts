...
export async function GET(request: Request) {
  ...
  const ok = () => NextResponse.redirect(new URL("/dashboard/settings?tab=email&email=microsoft_connected", site));
  ...
  try {
    const token = await exchangeMicrosoftEmailCode(code);
    const profile = await fetchMicrosoftEmailProfile(token.access_token);
    ...
    const { error } = await supabase.from("company_social_connections").upsert(
      {
        ...
        status: "connected",
        display_name: profile.name ?? profile.email ?? "Microsoft E-mail",
